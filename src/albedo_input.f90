! Read albedo values from the preprocessing into EBM main program

      subroutine albedo_input(Pcoalbedo, filename)
...
!     This is the name of the data file we will read.
      logical Albedo_2D
      character*(*) filename
!      parameter (filename='../input/albedo.nc')
      integer ncid
      real error 
...
!     Now everything worked as expected
      print *,'SUCCESSFULLY read albedo from [', filename,'] into simulation!'
!      write(2,*)'SUCCESSFULLY read albedo from [', filename,'] into simulation!'
      end
